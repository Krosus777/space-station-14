    ...
    public sealed partial class PaperWindow : BaseWindow
    {
        ...
        public PaperWindow()
        {
            ...
            Input.OnKeyBindDown += args => // Solution while TextEdit don't have events
            {
                ...
                else if (args.Function == ContentKeyFunctions.OpenPaperRadialMenu)
                {
                    OpenFieldMenu();
                    args.Handle();
                }
        ...
        private void OpenFieldMenu()
        {
            if (!_entMan.TryGetComponent(OwnerUid, out PaperFormComponent? form))
                return;

            var menu = new SimpleRadialMenu();
            var buttons = new List<RadialMenuOption>
            {
                new RadialMenuActionOption<object?>(_ => OpenPicker(form, "[FIO]"), null)
                { ToolTip = Loc.GetString("paper-radial-name") },
                new RadialMenuActionOption<object?>(_ => OpenPicker(form, "[JOB]"), null)
        ...
        private void OpenPicker(PaperFormComponent form, string placeholder)
        {
            if (!form.Dropdown.TryGetValue(placeholder, out var options))
                options = new List<string>();

            var picker = new FormFieldPickerWindow();
            picker.Populate(options);
            picker.OnConfirmed += text => Input.InsertAtCursor(text);
            picker.OpenCentered();
        }
